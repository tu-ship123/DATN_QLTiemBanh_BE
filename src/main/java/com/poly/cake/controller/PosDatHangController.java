...
import com.poly.cake.service.PosDatHangService;
...
public class PosDatHangController {

    @Autowired
    private PosDatHangService posOrderService;

    // API Tạo hóa đơn tại quầy + Lấy mã chuyển khoản VietQR + Dữ liệu in bill
    @PostMapping
    public ResponseEntity<?> createPosOrder(@Valid @RequestBody PosDatHangDto.Request request, Authentication authentication) {
        // Lấy email của Nhân viên thu ngân đang thực hiện đăng nhập từ Token
        String emailNhanVien = authentication.getName();

        PosDatHangDto.Response response = posOrderService.createPosOrder(request, emailNhanVien);
