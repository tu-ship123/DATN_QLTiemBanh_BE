...
public class BoXuLyNgoaiLeToanCuc {
    ...
    @ExceptionHandler(NgoaiLeKhongTimThayTaiNguyen.class)
    public ResponseEntity<?> handleNotFound(NgoaiLeKhongTimThayTaiNguyen e) {
        return ResponseEntity.status(HttpStatus.NOT_FOUND).body(Map.of("message", e.getMessage()));
    }

    // Lỗi 400: Sai logic nghiệp vụ
    @ExceptionHandler(NgoaiLeNghiepVu.class)
    public ResponseEntity<?> handleBusiness(NgoaiLeNghiepVu e) {
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(Map.of("message", e.getMessage()));
    }

    // Lỗi 403: Không đủ quyền (do mình tự throw trong Service)
    @ExceptionHandler(NgoaiLeCamTruyCap.class)
    public ResponseEntity<?> handleForbidden(NgoaiLeCamTruyCap e) {
