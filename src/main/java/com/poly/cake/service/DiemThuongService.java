...
public class DiemThuongService {
    ...
    public DiemVoucherDto.VoucherResponse doiDiem(String email, DiemVoucherDto.DoiDiemRequest request) {
        ...
        MaGiamGia ma = maGiamGiaRepository
                ...
                .orElseThrow(() -> new RuntimeException("Mã giảm giá không hợp lệ hoặc không thể đổi bằng điểm!"));

        int tongDiem = diemThuongRepository.tinhTongDiem(khach);
        if (tongDiem < ma.getDiemCanDung()) {
            throw new RuntimeException("Không đủ điểm! Bạn có " + tongDiem + " điểm, cần " + ma.getDiemCanDung() + " điểm.");
        }
    ...
    public void congDiemXacNhanNhanHang(Long donHangId, String email) {
        ...
        if (diemThuongRepository.existsByKhachHangAndDonHangIdAndLoaiGiaoDich(khach, donHangId, "MUAT_HANG_ONLINE")) {
            throw new RuntimeException("Đơn hàng này đã được cộng điểm rồi!");
        }
    ...
    public DiemVoucherDto.CongDiemPosResponse congDiemPos(DiemVoucherDto.CongDiemPosRequest request) {
        ...
        DonHang donHang = donHangRepository.findById(request.getDonHangId())
                .orElseThrow(() -> new RuntimeException("Đơn hàng POS không tìm thấy!"));

        if (diemThuongRepository.existsByKhachHangAndDonHangIdAndLoaiGiaoDich(khach, donHang.getId(), "MUAT_HANG_POS")) {
