...
import com.poly.cake.dto.XacThucDto.*;
...
public class XacThucService {
    ...
    private final DichVuTokenRedis redisTokenService;
    private final DichVuTotp totpService;
    ...
    public void register(RegisterRequest request) {
        ...
        if (nguoiDungRepository.existsByEmail(email)) {
            throw new NgoaiLeNghiepVu("Email đã được sử dụng!");
        }
        ...
        if (soDienThoai != null && !soDienThoai.isBlank()
            ...
            throw new NgoaiLeNghiepVu("Số điện thoại đã được sử dụng!");
        }
        ...
        try {
            ...
            throw e; // Vi phạm ràng buộc khác -> để BoXuLyNgoaiLeToanCuc xử lý (fallback 409 chung)
        }
    }
    ...
    public AuthResponse login(LoginRequest request) {
        ...
        try {
            ...
            throw new NgoaiLeNghiepVu("Email hoặc mật khẩu không chính xác!");
        }
        ...
        if ("BI_KHOA".equals(user.getTrangThai())) {
            throw new NgoaiLeNghiepVu("Tài khoản của bạn đã bị khóa. Vui lòng liên hệ quản trị viên để được hỗ trợ!");
        }
        if ("NGUNG_HOAT_DONG".equals(user.getTrangThai())) {
            throw new NgoaiLeNghiepVu("Tài khoản của bạn đã ngừng hoạt động!");
        }
        if (!"HOAT_DONG".equals(user.getTrangThai())) {
            throw new NgoaiLeNghiepVu("Tài khoản không ở trạng thái hoạt động, vui lòng liên hệ quản trị viên!");
        }

        // Xử lý 2FA (TOTP)
        if (Boolean.TRUE.equals(user.getIs2FaEnabled())) {
            if (request.getTotpCode() == null || request.getTotpCode().isBlank()) {
                throw new NgoaiLeNghiepVu("Vui lòng nhập mã xác thực 2 bước (2FA)!");
            }
            if (!totpService.verifyCode(user.getTotpSecret(), request.getTotpCode())) {
                throw new NgoaiLeNghiepVu("Mã xác thực 2 bước không chính xác!");
            }
        }
    ...
    public AuthResponse refreshToken(String refreshToken) {
        LamMoiToken savedToken = lamMoiTokenRepository.findByToken(refreshToken)
                .orElseThrow(() -> new NgoaiLeNghiepVu("Refresh Token không hợp lệ!"));
        ...
        if (savedToken.getNgayHetHan().isBefore(LocalDateTime.now()) || !jwtUtil.isTokenValid(refreshToken)) {
            ...
            throw new NgoaiLeNghiepVu("Refresh Token đã hết hạn, vui lòng đăng nhập lại!");
        }
    ...
    public void logout(String accessToken, String email) {
        NguoiDung user = nguoiDungRepository.findByEmail(email)
                .orElseThrow(() -> new NgoaiLeNghiepVu("Lỗi xác thực người dùng!"));
    ...
    public void resetPassword(ResetPasswordRequest request) {
        ...
        if (user.getOtpHetHan() == null || user.getOtpHetHan().isBefore(LocalDateTime.now())) {
            throw new NgoaiLeNghiepVu("Mã OTP đã hết hạn, vui lòng yêu cầu mã mới!");
        }

        if (user.getMaOtp() == null || !user.getMaOtp().equals(request.getOtp())) {
            throw new NgoaiLeNghiepVu("Mã OTP không chính xác!");
        }
    ...
    public AuthResponse loginWithGoogle(GoogleLoginRequest request) {
        ...
        if (googleId == null || email == null) {
            throw new NgoaiLeNghiepVu("Không lấy được thông tin tài khoản Google!");
        }
        if (!"true".equalsIgnoreCase(emailVerified)) {
            throw new NgoaiLeNghiepVu("Email Google chưa được xác thực!");
        }
    ...
    private Map<String, Object> verifyGoogleIdToken(String idToken) {
        ...
        try {
            ...
            throw new NgoaiLeNghiepVu("Google idToken không hợp lệ hoặc đã hết hạn!");
        } catch (Exception e) {
            log.error("Lỗi khi xác thực Google idToken: {}", e.getMessage());
            throw new NgoaiLeNghiepVu("Không thể xác thực tài khoản Google, vui lòng thử lại!");
        }

        if (payload == null) {
            throw new NgoaiLeNghiepVu("Google idToken không hợp lệ!");
        }

        Object aud = payload.get("aud");
        if (aud == null || !googleClientId.equals(aud.toString())) {
            throw new NgoaiLeNghiepVu("idToken không được cấp cho ứng dụng này!");
        }
    ...
    public TotpSetupResponse setupTotp(String email) {
        ...
        if (Boolean.TRUE.equals(user.getIs2FaEnabled())) {
            throw new NgoaiLeNghiepVu("Tài khoản đã được bật 2FA từ trước!");
        }
    ...
    public void verifyAndEnableTotp(String email, String code) {
        ...
        if (user.getTotpSecret() == null) {
            throw new NgoaiLeNghiepVu("Vui lòng thiết lập 2FA trước khi xác minh!");
        }

        if (!totpService.verifyCode(user.getTotpSecret(), code)) {
            throw new NgoaiLeNghiepVu("Mã xác thực không chính xác!");
        }
    ...
    public void disableTotp(String email) {
        ...
        if (!Boolean.TRUE.equals(user.getIs2FaEnabled())) {
            throw new NgoaiLeNghiepVu("Tài khoản chưa bật 2FA!");
        }
